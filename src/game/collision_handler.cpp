...
bool checkCollision(const engine::Rect& rectangleLeft,
...
bool IsColliding(engine::Entity const& Lhs, engine::Entity const& Rhs) {
    return std::visit(
        engine::overloads{
            [&](const engine::Circle& lhsCircle,
                const engine::Circle& rhsCircle) {
                return checkCollision(lhsCircle, *Lhs.transform, rhsCircle,
                                      *Rhs.transform);
            },
            [&](const engine::Rect& lhsRect, const engine::Rect& rhsRect) {
                return checkCollision(lhsRect, *Lhs.transform, rhsRect,
                                      *Rhs.transform);
            },
            [&](const engine::Circle& lhsCircle, const engine::Rect& rhsRect) {
                return checkCollision(lhsCircle, *Lhs.transform, rhsRect,
                                      *Rhs.transform);
            },
            [&](const engine::Rect& lhsRect, const engine::Circle& rhsCircle) {
                return checkCollision(rhsCircle, *Rhs.transform, lhsRect,
                                      *Lhs.transform);
            }},
